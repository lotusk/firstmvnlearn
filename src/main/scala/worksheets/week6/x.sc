...
object x {
	...
	charCode('E')                             //> res0: Char = 3
	def wordCode(word: String): String =
		word.toUpperCase map charCode     //> wordCode: (word: String)String
	// mnem map[(Char,String),String]((digit,str)=>"x")

	wordCode("Java")                          //> res1: String = 5282

	val wordForSum: Map[String, Seq[String]] =
		words groupBy wordCode withDefaultValue Seq()
                                                  //> wordForSum  : Map[String,Seq[String]] = Map(63972278 -> List(newscast), 2923
	...
	def encode(number: String): Set[List[String]] =
		if (number.isEmpty()) Set(List())
			...
			for {
				...
				word <- wordForSum(number take split)
				rest <- encode(number drop split)
			} yield word :: rest
		}.toSet                           //> encode: (number: String)Set[List[String]]

	encode("7225247386")                      //> res2: Set[List[String]] = Set(List(sack, ah, re, to), List(rack, bird, to),
